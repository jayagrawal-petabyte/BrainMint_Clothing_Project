...
const AdminTopbar = ({ toggleSidebar }) => {
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      ...
      if (token) {
        ...
        // Also fetch contact messages
        const msgResponse = await fetchContactMessages(token);
        if (msgResponse && msgResponse.success !== false) {
          const msgs = msgResponse.data?.contacts || msgResponse.contacts || msgResponse.data || [];
          // Show the latest 3 unread/recent messages
          setNewMessages(Array.isArray(msgs) ? msgs.slice(0, 3) : []);
        }
      }
    };
    loadNotifications();
  ...
  const totalNotifications = pendingOrders.length + newMessages.length;
          ...
          {isNotificationOpen && (
            <div className="absolute right-0 mt-3 w-80 bg-admin-card dark:bg-admin-card-dark border border-admin-border dark:border-admin-border-dark rounded-xl shadow-lg overflow-hidden font-rubik z-50">
              <div className="p-4 border-b border-admin-border dark:border-admin-border-dark flex justify-between items-center bg-admin-bg/30 dark:bg-[#1A1A1A]/30">
                ...
                <span className="text-xs bg-admin-accent text-white px-2 py-0.5 rounded-full">{totalNotifications} New</span>
              </div>
                ...
                {totalNotifications === 0 ? (
                  ...
                  <>
                    ...
                    {newMessages.map((msg, i) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-1">
                          <span className="font-semibold text-sm text-admin-heading dark:text-admin-heading-dark">New Contact Message</span>
                          <span className="text-xs text-blue-500 font-medium">Message</span>
                        </div>
                        <p className="text-xs text-admin-text dark:text-admin-text-dark truncate">
                          {msg.name || 'Unknown'}: {msg.message?.slice(0, 60) || msg.subject || '...'}
                        </p>
                        <span className="text-[10px] text-admin-text/70 dark:text-admin-text-dark/70 mt-2 block">
                          {msg.createdAt ? new Date(msg.createdAt).toLocaleString('en-IN') : 'Recently'}
                        </span>
                      </Link>
                    ))}
                  </>
              ...
              {totalNotifications > 0 && (
                <div className="grid grid-cols-2 border-t border-admin-border dark:border-admin-border-dark">
                  <Link 
                    to="/admin/orders" 
                    onClick={() => setIsNotificationOpen(false)}
                    className="block p-3 text-center text-sm text-admin-accent hover:bg-admin-bg/30 dark:hover:bg-[#1A1A1A]/30 font-medium transition-colors border-r border-admin-border dark:border-admin-border-dark"
                  >
                    View Orders
                  </Link>
                  <Link 
                    to="/admin/messages" 
                    onClick={() => setIsNotificationOpen(false)}
                    className="block p-3 text-center text-sm text-admin-accent hover:bg-admin-bg/30 dark:hover:bg-[#1A1A1A]/30 font-medium transition-colors"
                  >
                    View Messages
                  </Link>
                </div>
