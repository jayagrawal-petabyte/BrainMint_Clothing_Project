          ...
          <tbody className="divide-y divide-admin-border dark:divide-admin-border-dark">
            {filteredProducts.map((product) => (
              <tr key={product.id} className="hover:bg-admin-bg/50 dark:hover:bg-[#222]/50 transition-colors">
                ...
                <td className="px-6 py-4 text-admin-text dark:text-admin-text-dark">{product.stock}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border
                    ${product.status === 'Active' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800' : 'bg-gray-50 text-gray-600 border-gray-200 dark:bg-gray-800 dark:border-gray-700'}
                  `}>
                    {product.status}
                  </span>
                </td>
                <td className="px-6 py-4 text-right text-sm font-medium space-x-3">
                  ...
                  <Link to={`/admin/products/edit/${product.id}`} className="text-admin-text hover:text-emerald-500 dark:text-admin-text-dark dark:hover:text-emerald-400 transition-colors inline-block" title="Edit">
                    <Edit2 size={18} />
                  </Link>
                  <button className="text-admin-text hover:text-red-500 dark:text-admin-text-dark dark:hover:text-red-400 transition-colors" title="Delete">
                    <Trash2 size={18} />
                  </button>
                </td>
              </tr>
